...
topo_map = {}
search(:node, "cassandra_cluster_name:#{node[:cassandra][:cluster_name]} AND ec2:placement_availability_zone").map do |n|
  az_parts = n[:ec2][:placement_availability_zone].split('-')
  topo_map[n['ipaddress']] = {
    :rack => az_parts.last[1, 1],
    :dc => az_parts[0..1].join('-') + "-#{az_parts.last[0, 1]}"}
end

node.set_unless[:cassandra][:ec2_snitch_default_az] = "us-east-1a"
default_az_parts = node[:cassandra][:ec2_snitch_default_az].split('-')
default_az = {:rack => default_az_parts.last[1, 1], :dc => default_az_parts[0..1].join('-') + "-#{default_az_parts.last[0, 1]}"}
